...
function Home({ navigate, loggedIn }) {
  ...
  return (
    <div className="w-full min-h-screen bg-forest font-sans text-parchment">
      <Navbar logoSrc={logo} links={links} onNavigate={navigate} />
      <section className="relative overflow-hidden border-b border-[#24513B] bg-[radial-gradient(circle_at_12%_70%,rgba(37,117,66,0.22),transparent_24%),linear-gradient(115deg,#073523,#022218)] px-6 py-12 text-parchment sm:px-10 lg:px-14 lg:py-12">
        ...
        <div className="relative w-full grid items-center gap-12 min-[900px]:grid-cols-[0.92fr_1.08fr] min-[1100px]:gap-16">
      ...
      <div className="border-b border-[#24513B] bg-[#06291D] px-6 py-4 sm:px-10 lg:px-14"><div className="flex w-full flex-wrap items-center justify-between gap-4 text-sm text-[#BFD5C3]"><span className="inline-flex items-center gap-2"><Leaf size={18} className="text-[#86D84A]" /> Trusted by farmers and researchers</span><span className="text-[#86D84A]">ICAR</span><span>IARI</span><span>IHR</span><span>Krishi Vigyan Kendra</span><span className="inline-flex items-center gap-2"><span className="rounded-full bg-[#368D43] px-3 py-1 text-xs font-semibold text-white">+250</span> Farmers and growing</span></div></div>
      <main id="resources" className="w-full px-6 py-12 text-parchment sm:px-10 lg:px-14"><section><p className="mb-4 text-sm font-semibold text-[#F5F0E1]">How a scan works</p><div className="relative grid grid-cols-3 gap-6 min-[700px]:flex min-[700px]:justify-between min-[700px]:gap-2"><div className="pointer-events-none absolute left-4 right-4 top-[15px] hidden border-t border-dashed border-parchment/50 min-[700px]:block" />{workflow.map((step, index) => <div key={step} className="relative z-10 flex flex-col items-center gap-2 text-center min-[700px]:flex-1"><span className="grid h-[30px] w-[30px] place-items-center rounded-full bg-turmeric text-sm font-semibold text-forestDeep shadow-[0_2px_0_#B5791C]">{index + 1}</span><span className="text-xs font-medium text-parchment">{step}</span></div>)}</div></section><section id="about" className="mt-14"><p className="mb-4 text-sm text-parchment">Where to go</p><div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">{destinations.map((destination) => <article key={destination.title} className="flex flex-col justify-between rounded-xl bg-parchmentDark p-[22px] shadow-[5px_5px_0_#1F3D2B]"><div className="mb-7 grid h-10 w-10 place-items-center rounded-full bg-forest text-[#D4EA9A]">{destination.icon}</div><div><h2 className="font-serif text-[21px] font-medium text-soil">{destination.title}</h2><p className="mt-3 text-sm leading-relaxed text-soilMuted">{destination.description}</p></div><button type="button" onClick={() => navigate(loggedIn || destination.screen === 'research' ? destination.screen : 'login')} className="mt-6 inline-flex items-center gap-1 text-sm font-semibold text-turmericDeep hover:text-rust">Open <ArrowRight size={15} /></button></article>)}</div></section></main>
    </div>
  );
}
